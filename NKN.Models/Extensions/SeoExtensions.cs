...
namespace NKN.Models.Extensions
{
    public static class SeoExtensions
    {
        ...
        public static string GetFormattedSeoTitle(this ISeo seo, IPage currentPage, string brandName,
            string format = "{0} | {1}", bool discardTitleForHomePage = true)
        {
            if (discardTitleForHomePage && currentPage.IsHome()) return brandName;

            var seoTitle = seo.SeoTitle.IsNullOrWhiteSpace() ? currentPage.PageTitle() : seo.SeoTitle;
        ...
        public static string GetCanonicalUrl(this ISeo seo, string canonicalDomain)
        {
            ...
            if (canonicalDomain.IsNullOrWhiteSpace()) return seo.Url(mode: UrlMode.Absolute);
