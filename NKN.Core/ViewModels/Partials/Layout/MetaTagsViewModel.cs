...
namespace NKN.Core.ViewModels.Partials.Layout
{
    public class MetaTagsViewModel
    {
        public MetaTagsViewModel(ISeoContext<ISeo> context)
        {
            ...
            AlternateLanguageLinks = context.Seo.AlternateLanguages?.Select(al =>
                new AlternateLinkViewModel(al, context.CurrentPage.Url(mode: UrlMode.Absolute))).ToList();
            HideFromSearchEngines =
                context.SiteSettings.HideAllPagesFromSearchEngines || context.Seo.HideFromSearchEngines;
