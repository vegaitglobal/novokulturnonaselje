...
namespace NKN.Core.Contexts
{
    public class SiteContext : ISiteContext
    {
        public SiteContext(UmbracoHelper umbracoHelper)
        {
			...
			LazyNews = new Lazy<IEnumerable<NewsDetails>>(() => UmbracoHelper.ContentAtXPath($"//{NewsDetails.ModelTypeAlias}")?.OfType<NewsDetails>().OrderByDescending(d=>d.ReleaseDate).Take(3));
		}
